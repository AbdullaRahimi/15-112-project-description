...
class DrawingPanel(Tk):
    ...
    '''
    def __init__(self, width=500, height=500, background="cyan"):
        Tk.__init__(self)
        self.width = width
        self.height = height
        self.title("Projectile Motion Simulator")
        self.canvas = Canvas(self, width = width + 1, height = height + 1)
        self.canvas["bg"] = background
        self.bg = background
        self.canvas.pack({"side": "top"})
        #self.label = Label(self, text = 'The time of flight is ' + str(totalTime))
        #self.label.pack()
